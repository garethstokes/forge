...
INSERT INTO outputs (id, run, example, text, error, latency_ms, tokens) VALUES
  ...
  (4, 1, 4, 'Tokyo.', NULL, 388, '{"input": 18, "output": 4}'),
  -- run 2 (capital-de errored; capital-au is correct this time)
  (5, 2, 1, 'Paris.', NULL, 210, '{"input": 18, "output": 3}'),
  (6, 2, 2, NULL, 'upstream timeout after 30s', 30000, NULL),
  (7, 2, 3, 'Canberra is the capital of Australia.', NULL, 540, '{"input": 18, "output": 8}'),
  (8, 2, 4, 'Tokyo.', NULL, 305, '{"input": 18, "output": 4}');
...
INSERT INTO scores (id, output, grader_version, value, passed, detail, error, created_at) VALUES
  ...
  (1, 1, 1, 1.0, true,  '{"rationale": "names Paris"}',              NULL,                      now() - interval '24 hours'),
  (2, 2, 1, 1.0, true,  '{"rationale": "names Berlin"}',             NULL,                      now() - interval '24 hours'),
  (3, 3, 1, 0.0, false, '{"rationale": "Sydney is not the capital"}',NULL,                      now() - interval '24 hours'),
  (4, 4, 1, 1.0, true,  '{"rationale": "names Tokyo"}',              NULL,                      now() - interval '24 hours'),
  -- run 2: fr✓ de=errored au✓(Canberra) jp✓  → μ 1.0, pass 100% (3 scored)
  (5, 5, 1, 1.0, true,  '{"rationale": "names Paris"}',              NULL,                      now() - interval '30 minutes'),
  (6, 6, 1, NULL, NULL, NULL,                                        'no output text to grade', now() - interval '30 minutes'),
  (7, 7, 1, 1.0, true,  '{"rationale": "names Canberra"}',           NULL,                      now() - interval '30 minutes'),
  (8, 8, 1, 1.0, true,  '{"rationale": "names Tokyo"}',              NULL,                      now() - interval '30 minutes');
...
INSERT INTO run_metrics (id, run, grader_version, mean, pass_rate, count, computed_at, tag, stderr) VALUES
  (1, 1, 1, 0.75, 0.75, 4, now() - interval '24 hours',  NULL, NULL),
  (2, 2, 1, 1.0,  1.0,  3, now() - interval '30 minutes', NULL, NULL);
...
INSERT INTO run_metrics (id, run, grader_version, mean, pass_rate, count, computed_at, tag, stderr) VALUES
  (3, 1, 2, 0.70, NULL, 4, now() - interval '24 hours', NULL,               0.07),
  (4, 1, 2, 0.75, NULL, 2, now() - interval '24 hours', 'theme:europe',     0.05),
  (5, 1, 2, 0.50, NULL, 1, now() - interval '24 hours', 'theme:oceania',    NULL),
  (6, 1, 2, 0.80, NULL, 1, now() - interval '24 hours', 'theme:asia',       NULL),
  (7, 1, 2, 0.75, NULL, 4, now() - interval '24 hours', 'axis:accuracy',    0.22),
