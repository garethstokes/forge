...
tutorial it would break a test, so these examples cannot silently rot. (The
suite, like the library, needs a Postgres; it spins up an ephemeral one per run.)

- **[Unit of Work](Tutorial/UnitOfWork.lhs)** — edit a plain value, get a minimal
  `UPDATE`. The snapshot-diff at the heart of the session.
- **[Relationships](Tutorial/Relationships.lhs)** — load related rows two ways:
  the A-path (`load`) and the D-path (`Ent` / `with` / `rel`), and `selectin` vs
  `joined`.
- **[Cascades](Tutorial/Cascades.lhs)** — delete a parent and its children go too:
  onDelete policies declared on the parent entity and applied at flush.
